...
resource "aws_iam_role" "eks-master-role" {
  name = "${var.name}-${var.environment}-eks-master-iam-role"
...
resource "aws_iam_role" "eks-node-role" {
  name = "${var.name}-${var.environment}-eks-node-iam-role"
...
resource "aws_iam_role_policy" "s3_readonly_policy" {
  name = "${var.name}-${var.environment}-eks-s3-policy"
  role = "${aws_iam_role.eks-node-role.id}"
...
resource "aws_iam_role_policy" "ec2_fullaccess_policy" {
  name = "${var.name}-${var.environment}-eks-ec2-policy"
  role = "${aws_iam_role.eks-node-role.id}"
